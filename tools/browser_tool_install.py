...
from hermes_constants import agent_browser_runnable, get_hermes_home, is_termux as _is_termux_environment
...
def _agent_browser_candidates(extended_path: str):
    _bt = _origin()
    for path in (shutil.which("agent-browser", path=None), shutil.which("agent-browser", path=extended_path)):
        if path:
            yield path
    yield from _bt._agent_browser_candidates(extended_path) if hasattr(_bt, "_agent_browser_candidates") else ()


node_tool_runnable = lambda path: _agent_browser_candidate_present(path)
